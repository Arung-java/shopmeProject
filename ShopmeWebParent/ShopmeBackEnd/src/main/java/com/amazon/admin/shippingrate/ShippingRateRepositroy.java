...
import com.amazon.admin.paging.SearchRepository;
import com.amazon.common.entity.Country;
import com.amazon.common.entity.ShippingRate;

public interface ShippingRateRepositroy extends SearchRepository<ShippingRate, Integer> {
	@Query("SELECT sr FROM ShippingRate sr WHERE sr.country.id =?1 AND sr.state = ?2")
	public ShippingRate findByCountryAndState(Integer usa,String state);
